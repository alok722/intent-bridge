...
export default function ScenarioSidebar() {
  ...
  return (
    <aside className="w-full md:w-64 bg-zinc-950 border-b md:border-r border-zinc-800 p-4 shrink-0 flex flex-col h-full overflow-y-auto">
      <div className="flex items-center gap-2 px-2 py-4 mb-4 border-b border-zinc-800">
        ...
        <h1 className="font-bold text-lg tracking-tight text-zinc-100">IntentBridge Core</h1>
      </div>
          ...
          {SCENARIOS.map((s) => {
            ...
            return (
              <Button
                ...
                className={cn(
                  "justify-start text-sm tracking-tight transition-all py-6 md:py-4 shrink-0 px-4 md:px-3 focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2",
                  isActive 
                    ? "bg-zinc-800/80 text-white font-medium shadow-inner border border-zinc-700/50" 
                    : "text-zinc-400 hover:bg-zinc-900 hover:text-zinc-200 border border-transparent"
                )}
              >
                <Icon className={cn("w-5 h-5 mr-3 hidden sm:block", isActive ? s.color : "text-zinc-500")} />
                {s.label}
              </Button>
            );
          })}
